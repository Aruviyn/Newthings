...
### git checkout
You may checkout any branch but it is best to leave your branch clean before checking out other branch.

```bash
~/checkouts/Newthings >>> git checkout issue-3                                                                                                                                                                    
...
Cherry picking facilitate porting over changes between branches. This will avoid introducing the same changes multiple times (which will cause conflicts). Since we already know how to reference any commits with their hashes, use the hash to port over changes by using cherry picking techniques:

```bash
~/checkouts/Newthings >>> git cherry-pick a9bae82                                                                                                                                                                 
[issue-3 7394f97] issue-3a just showing git cherry picking
 Date: Thu Aug 26 00:54:27 2021 +0800
 1 file changed, 46 insertions(+), 8 deletions(-)
~/checkouts/Newthings >>>                         
```

If you mess up during cherry picking, it is possible to abort the cherry picking and review back your actions
