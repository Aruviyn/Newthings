...
---
## File manipulation.
- File manipulation is an important aspect in working with a system especially when you are going to work with terminal interface
- Below are the tasks with description using the command in terminal: 

| Task | Description | Command | 
|---|---|---|
| Show the present working directory | The command prints the full present working directory path, the directory we are looking at | `pwd` |
| list down content of a directory | The command list down the content of the directory listing file permissions and owner of the items | `ls` |
| make a recursive directory | The command created a directory recursively given a path | `mkdir -p folder/file/subfile |
| write an output into a file | The command will pipe the output of a program into another file, this way the output of a file can be written directly into another file to be consumed/concatenated into another file for later use. There are two ways this manipulation can be done, 1) append the file with the output or 2) overwrite the file with the output. | Writing or overwriting in a file example: `df -h > test.txt` , Appending in a file eaxmple: `df -h >> test.txt` |
| read/peek at the end of the file | Seldom comes time where we would seek to read the final lines of the files, what command is being used to output the end of the file. The commands are also manipulatable by having a -f switch to follow the output of the file, this way we could always see the new lines being written on the files, essential when reading logs | tail -n 1 test.txt |
